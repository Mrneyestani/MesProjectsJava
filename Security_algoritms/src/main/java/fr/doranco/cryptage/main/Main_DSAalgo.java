...
public class Main_DSAalgo {
	...
	public static void main(String args[]) {
		try {
			...
			byte[] signature = Cryptage_DSA.Create_Digital_Signature(messageByte, privateKey);
			...
			Cryptage_DSA.Verify_Digital_Signature(messageByte, signature, publicKey);
			...
			Cryptage_DSA.Verify_Digital_Signature(messageByte, signature2, publicKey2);

			System.out.println("--- Example with a invalid signature: using public key from another user ---");
			Cryptage_DSA.Verify_Digital_Signature(messageByte, signature, publicKey2);
